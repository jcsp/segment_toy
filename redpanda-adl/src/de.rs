...
impl<'de, DE: Decoder> Deserializer<'de, DE> {
    fn take_bytes<const N: usize>(&mut self) -> Result<[u8; N]> {
        self.input.claim_bytes_read(N)?;
        let mut bytes = [0u8; N];
        self.input.reader().read(&mut bytes)?;
        // debug!("take_bytes {:x?}", bytes);
        Ok(bytes)
    }
}

pub fn from_bytes<T, C: Config>(slice: &[u8], config: C) -> Result<T>
    where T: DeserializeOwned {
    let original_len = slice.len();
    let reader = SliceReader::new(slice);
    let mut decoder = bincode::de::DecoderImpl::new(reader, config);
    let deserializer = Deserializer { input: &mut decoder };
    let t = T::deserialize(deserializer)?;
    Ok(t)
    // if deserializer.input.is_empty() {
    //     Ok(t)
    // } else {
    //     error!("Trailing bytes ({} bytes leftover from {} buffer)", deserializer.input.len(), original_len);
...
impl<'a, 'de, DE: Decoder> serde::Deserializer<'de> for Deserializer<'a, DE> {
    ...
    fn deserialize_struct<V>(mut self, name: &'static str, fields: &'static [&'static str], visitor: V) -> Result<V::Value> where V: Visitor<'de> {
        ...
        impl<'de, 'a, 'b: 'a, DE: Decoder + 'b> SeqAccess<'de> for Access<'a, 'b, DE> {
                    ...
                    T: DeserializeSeed<'de>,
            {
                if self.len > 0 {
                    self.len -= 1;
                    let value = DeserializeSeed::deserialize(seed,
                                                             Deserializer { input: self.deserializer.input })?;
